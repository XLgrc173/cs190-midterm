...
func price(rentalCar: RentalCar, totalHours: Int) -> Float {
    _ = 6/7 // this you need to compute from totalHours
    _ = 6/24 // you also need to compute this from totalHours
    let dailyPrice = rentalCar.dailyPrice
    let hourlyPrice = rentalCar.hourlyPrice
    return 6/7 * dailyPrice + 6/24 * hourlyPrice // this return statement isn't quite right yet!
}
...
func pricingFunctionForCar(rentalCar: RentalCar) -> (Int) -> (Float) {
    func priceForHours(totalHours: Int) -> Float {
        return rentalCar.dailyPrice // with what you did in Part II A, fixing this return to return the right thing is easy
    }
